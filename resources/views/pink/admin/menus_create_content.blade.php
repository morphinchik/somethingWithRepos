<div id="content-page", class="content group">
	<div class="hentry group">
		{!! Form::open(['url' => (isset($menu->id)) ? route('admin.menus.update', ['menu' => $menu->id]) : route('admin.menus.store'), 'class' => 'contact-form', 'method' => 'POST', 'enctype' => 'multipart/form-data']) !!}
			@if(isset($menu->id))
			...
			<li class="text-field">
				...
				<div class="input-prepend">
					<span class="add-on"><i class="icon-user"></i></span>
						{!! Form::text('title', isset($menu->title) ? $menu->title : old('title'), ['placeholder' => 'Введите название стрницы']) !!}
				</div>
			</li>
			<li class="text-field">
				<label for="name-contact-us">
			...
			<div id="accordion">
				...
				<ul>
					<li class="text-field">
						...
						<div class="input-prepend">
							<span class="add-on"><i class="icon-user"></i></span>
								{!! Form::text('custom_link', (isset($menu->path) && $type == 'customLink') ? $menu->path : old('path'), ['placeholder' => 'Введите путь для ссылки']) !!}
						</div>
					</li>
...
<script>
	jQuery(function($) {
		...
		var active = 1;
		$( '#accordion input[type=radio]').each(function(ind, item) {
